...
import { buildFlashcardPrompt } from '@/lib/gateway/promptBuilder';
import { ok, fail, parseLLMJson } from '@/lib/gateway/envelope';
import { requireUser } from '@/lib/security/requireUser';
import { checkRateLimit } from '@/lib/security/rateLimit';
import { validateFlashcard } from '@/lib/security/validate';
import type { FlashcardRequest, FlashcardResponse } from '@/lib/types/gateway';
...
export async function POST(req: NextRequest) {
  ...
  const validationError = validateFlashcard(body);
  if (validationError) {
    return NextResponse.json(fail('BAD_REQUEST', validationError), { status: 400 });
  }

  const reqData = body as FlashcardRequest;
  const { system, user: userPrompt } = buildFlashcardPrompt(reqData);

  try {
    const config = getAIConfig('flashcard', user.tier);
    ...
    const parsed = parseLLMJson<FlashcardResponse>(raw);
